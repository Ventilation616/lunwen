...
概念结构设计采用实体-联系方法，通过绘制实体关系图描述系统中的数据实体及其相互关系。根据需求分析，本系统涉及的主要数据实体包括用户、运动类型、运动记录、系统配置、AI会话、运动知识、模型版本和矫正报告。以下采用陈氏ER图表示法，分别绘制各实体的属性图和整体实体联系图。

#### 4.2.1.1 用户实体属性图

用户实体存储系统用户的基本信息，主要属性包括用户标识、用户名、密码哈希、微信开放标识、头像、手机号、性别、年龄、身高、体重、昵称、角色、状态等。其中用户标识为主键，用下划线标注。
...
运动类型实体存储系统支持的运动项目信息，主要属性包括类型标识、运动名称、运动代码、状态等。其中类型标识为主键。
...
运动记录实体存储用户的健身运动数据，主要属性包括记录标识、用户标识、运动类型标识、运动时长、正确次数、错误次数、详细数据等。其中记录标识为主键，用户标识和运动类型标识为外键。
...
系统配置实体存储系统的配置参数，主要属性包括配置标识、配置键名、配置值、描述等。其中配置标识为主键。
...
AI会话实体存储用户与AI服务的对话记录，主要属性包括会话标识、用户标识、会话编号、消息数量等。其中会话标识为主键，用户标识为外键。
...
运动知识实体存储健身知识科普内容，主要属性包括知识标识、标题、分类、内容、状态等。其中知识标识为主键。
...
模型版本实体存储姿态识别模型的版本信息，主要属性包括版本标识、模型代码、模型名称、版本标签、准确率、延迟等。其中版本标识为主键。
...
矫正报告实体存储用户运动的分析报告，主要属性包括报告标识、用户标识、运动记录标识、总结文本、错误分布数据等。其中报告标识为主键，用户标识和运动记录标识为外键。
...
实体联系总图展示了系统中各实体之间的关联关系。用户实体与运动记录实体之间存在一对多的关系，一个用户可以有多条运动记录；用户实体与AI会话实体之间存在一对多的关系，一个用户可以有多条AI会话记录；用户实体与矫正报告实体之间存在一对多的关系，一个用户可以有多份矫正报告；运动类型实体与运动记录实体之间存在一对多的关系，一种运动类型对应多条运动记录；运动记录实体与矫正报告实体之间存在一对一的关系，每条运动记录可生成一份矫正报告。
...
graph TB
    ...
    style KA fill:#fff3e0,stroke:#ef6c00,stroke-width:2px
    style MV fill:#fff3e0,stroke:#ef6c00,stroke-width:2px
    style R1 fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    style R2 fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    style R3 fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    style R4 fill:#fce4ec,stroke:#c2185b,stroke-width:2px
    style R5 fill:#fce4ec,stroke:#c2185b,stroke-width:2px
```

上述实体联系图中，矩形表示实体，菱形表示实体间的联系，连线上的标注表示联系的基数。系统配置、运动知识、模型版本三个实体相对独立，不与其他实体产生直接关联，主要用于存储系统配置参数、健身知识科普内容和算法模型版本信息。
